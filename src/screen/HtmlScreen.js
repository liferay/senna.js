...
class HtmlScreen extends RequestScreen {
  ...
  copyNodeAttributesFromContent_(content, node) {
    ...
    localContent = localContent.replace(/[<]\s*html/gi, "<senna");
    localContent = localContent.replace(/\/html\s*>/gi, "/senna>");
    let placeholder;
    if (UA.isIe) {
      const tempNode = globals.document
        .createRange()
        .createContextualFragment(localContent);
      placeholder = tempNode.querySelector("senna");
    } else {
      node.innerHTML = localContent;
  ...
  makeTemporaryStylesHrefsUnique_() {
    const temporariesInDoc = this.virtualQuerySelectorAll_(
      HtmlScreen.selectors.stylesTemporary
    );
