...
class App extends EventEmitter {
	...
	constructor() {
		...
		this.appEventHandlers_.add(
			on(
				globals.window,
				'scroll',
				utils.debounce(this.onScroll_.bind(this), 100)
			),
	...
	doNavigate_(path, opt_replaceHistory) {
		...
		if (!route) {
			this.pendingNavigate = Promise.reject(
				new Error(`No route for ${path}`)
			);
	...
	getRoutePath(path) {
		...
		if (this.getIgnoreQueryStringFromRoutePath()) {
			...
			return utils.getUrlPathWithoutHashAndSearch(
				localPath.substr(this.basePath.length)
			);
		}

		localPath = utils.getUrlPathWithoutHash(localPath);
		return utils.getUrlPathWithoutHash(
			localPath.substr(this.basePath.length)
		);
	}
	...
	prepareNavigateSurfaces_(nextScreen, surfaces, params) {
		Object.keys(surfaces).forEach(id => {
			...
			console.log(
				`Screen [${nextScreen.getId()}] add content to surface [${
					surfaces[id]
