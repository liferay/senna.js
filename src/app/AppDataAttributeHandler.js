...
class AppDataAttributeHandler extends Disposable {
  ...
  handle() {
    ...
    if (!this.baseElement.hasAttribute(dataAttributes.senna)) {
      console.log(
        "Senna was not initialized from data attributes. In order to enable its usage from data attributes try setting in the base element, e.g. `<body data-senna>`."
      );
      return;
    }
  ...
  maybeSetLinkSelector_() {
    const linkSelector = this.baseElement.getAttribute(
  ...
  maybeSetLoadingCssClass_() {
    const loadingCssClass = this.baseElement.getAttribute(
  ...
  maybeSetUpdateScrollPosition_() {
    const updateScrollPosition = this.baseElement.getAttribute(
