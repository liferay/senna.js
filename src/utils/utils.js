...
class utils {
  ...
  static getCurrentBrowserPath() {
    return (
      this.getCurrentBrowserPathWithoutHash() + globals.window.location.hash
    );
  }
  ...
  static isCurrentBrowserPath(url) {
    if (url) {
      ...
      return (
        utils.getUrlPathWithoutHash(url) === this.getUrlPath(currentBrowserPath)
      );
    }
    return false;
  }
  ...
  static isChrome() {
    return (
      (utils.matchUserAgent("Chrome") || utils.matchUserAgent("CriOs")) &&
      !utils.isOpera() &&
      !utils.isEdge()
    );
  }
